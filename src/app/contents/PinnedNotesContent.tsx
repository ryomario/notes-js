...
function PinnedNotesContent({ open }: Readonly<ContentProps>) {
    const { t } = useTranslation()
    const {state:isGrid, setState:setIsGrid} = useSavedState<boolean>('last-isgrid-content-pinnednotes',true)
    const toggleGrid = () => {
        setIsGrid(old => !old)
    }
    if(!open)return null
    return (
        <Content open={open}>
            <div className="content-header">
                <div className="content-title">{t('content_pinnednotes_title')}</div>
